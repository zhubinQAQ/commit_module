from torchvision.datasets.folder import IMG_EXTENSIONS, DatasetFolder, default_loader, make_dataset
...
class ImageFolderList(DatasetFolder):
    def __init__(self, root_list, transform=None, target_transform=None, loader=default_loader):
        """
        Args:
            root_list (list[str] or tuple[str])
            transform (list[Callable], optional)
            target_transform (list[Callable], optional)
